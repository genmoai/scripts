...
def get_instance_params() -> typing.Tuple[str, str, str]:
    ...
    instance_id = requests.get(METADATA_URL + "id", headers=METADATA_HEADERS).text
    return project_id, zone, instance_id


def report_metrics(values: MetricsData, project_id: str, zone: str, instance_id: str) -> None:
    """
    ...
    point = monitoring_v3.Point({"interval": interval, "value": {"double_value": 0.0}})
    time_series.points = [point]

    project_name = "projects/{}".format(project_id)

    series = []
    gpu_bus_ids = list(sorted([gpu_bus_id for _, gpu_bus_id in values.keys()]))
    for (gpu_type, gpu_bus_id), metrics in values.items():
        for smi_metric, gcp_metric in METRICS.items():
            ...
            if len(gpu_bus_ids) > 1:
                new_series.metric.labels["gpu_bus_id"] = str(list(gpu_bus_ids).index(gpu_bus_id))
...
def get_metrics() -> MetricsData:
    ...
    process = subprocess.run(
        [NVIDIA_SMI_BIN, "--query-gpu={}".format(query_params), "--format=csv,noheader,nounits"],
    ...
    for line in process.stdout.decode().splitlines():
        line = line.split(",")
        data[tuple(line[:2])] = {k: v for k, v in zip(sorted(METRICS.keys()), map(float, line[2:]))}
...
def check_nvidia_smi() -> bool:
    ...
    """
    try:
        process = subprocess.run([NVIDIA_SMI_BIN, "-L"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except FileNotFoundError:
        print(
            "Couldn't find the nvidia-smi tool. Make sure it's properly " "installed in one of the directories in $PATH.", file=sys.stderr
        )
        return False
    ...
    if len(cards) == 0:
        print("The nvidia-smi tool didn't detect any GPUs attached to the " "system.", file=sys.stderr)
        return False
    return True
...
def main():
    ...
    try:
        ...
        print(
            "Couldn't connect with the metadata server. Are you sure you are " "executing this script on Google Compute Engine instance?",
            file=sys.stderr,
        )
